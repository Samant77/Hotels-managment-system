 ...
 public function Bookings()
{
    ...
    // Initialize an array to store payments related to bookings
    $payments = [];

    // Loop through each booking to fetch its associated payments
    foreach ($bookings as $booking) {
        // Fetch payments related to this booking and store them in the $payments array
        $payment = Payment::where('booking_id', $booking->id)->first();
        // Check if payment exists before adding it to the array
        if ($payment) {
            $payments[$booking->id] = $payment;
        }
    }

    return view('Backend.Room.bookings', compact('bookings', 'admin', 'payments'));
}
